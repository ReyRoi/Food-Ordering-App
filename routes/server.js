...
app.delete('/deleteMenu/:id',async(req,res)=>{
    const {id}=req.params
    const del=await menuModel.findByIdAndDelete(id)
    if(del){
...
app.put('/editmenu/:id',async(req,res)=>{
    const {name,type,price,description}=req.body
    console.log(req.body)
    const {id}=req.params
    const menu=await menuModel.findByIdAndUpdate(id,{name:name,type:type,des:description,price:price})
    if(menu){
...
app.get('/getsinglemenu/:id',async(req,res)=>{
    const {id}=req.params
    const menu=await menuModel.findById(id)
    res.status(200).json(menu)
})
app.post('/closeorder/:id',async(req,res)=>{
    const {id}=req.params
    const orders=new totalModel({orders:req.body.items})
    await orders.save()
    const deleteItems=await orderModel.find({table:id}).deleteMany()
    const tableStatus=await tableModel.findOne({id:id}).updateOne({status:"order completed"})
    if(deleteItems){
...
app.delete('/deletesingleorder/:id',async(req,res)=>{
    const {id}=req.params
    const deleteItem=await orderModel.findById(id).deleteOne()
    if(deleteItem){
        res.status(201).json("deleted")
    }
})
